    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="px-6 pt-24 pb-20 max-w-7xl mx-auto">
        <ScrollReveal direction="up">
          <span className="text-xs tracking-[0.25em] uppercase text-muted-foreground" style={{ fontFamily: 'var(--font-body)' }}>
            Training Systems
          </span>
        </ScrollReveal>
      ...
      <section className="px-6 pb-32 max-w-7xl mx-auto">
          ...
          {programs.map((program, i) => (
            <ScrollReveal key={program.title} direction="up" delay={i * 120} threshold={0.1}>
              <div className="liquid-glass rounded-2xl p-8 h-full flex flex-col border border-border/40 hover:border-border/70 transition-colors duration-300">
                <span className="text-[10px] tracking-[0.3em] uppercase text-muted-foreground mb-4" style={{ fontFamily: 'var(--font-body)' }}>{program.tag}</span>
                <h2 className="text-3xl font-normal mb-4 text-foreground" style={{ fontFamily: 'var(--font-display)' }}>{program.title}</h2>
                <p className="text-muted-foreground text-sm leading-relaxed mb-6 flex-1" style={{ fontFamily: 'var(--font-body)' }}>{program.description}</p>
                <ul className="space-y-2 mb-8">
                  {program.features.map((f) => (
                    <li key={f} className="flex items-center gap-2 text-sm text-foreground/80" style={{ fontFamily: 'var(--font-body)' }}>
                      <span className="w-1 h-1 rounded-full bg-foreground/40 shrink-0" />
                      {f}
                    </li>
                  ))}
                </ul>
                <div className="flex items-center justify-between text-xs text-muted-foreground border-t border-border/30 pt-5" style={{ fontFamily: 'var(--font-body)' }}>
        ...
        <ScrollReveal direction="up" delay={0} threshold={0.3}>
          <div className="mt-20 text-center">
            <p className="text-muted-foreground text-base mb-6" style={{ fontFamily: 'var(--font-body)' }}>Not sure which program is right for you?</p>
            <Link href="/contact">
              <button className="liquid-glass rounded-full px-12 py-4 text-sm text-foreground transition-transform duration-200 hover:scale-[1.03] cursor-pointer" style={{ fontFamily: 'var(--font-body)' }}>
